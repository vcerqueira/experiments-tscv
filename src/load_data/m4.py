...
class M4Dataset(LoadDataset):
    ...
    def load_data(cls, group, min_n_instances=None, extended: bool = False):
        ...
        ds_og = cls._load_data(group=group, min_n_instances=min_n_instances)

        if extended:
            if group == 'Daily':
                ds_h = cls._load_data(group='Hourly', min_n_instances=min_n_instances)
                ds_h_d = ds_h.groupby('unique_id').resample(on='ds', rule='D').mean().reset_index()

                ds = pd.concat([ds_og, ds_h_d]).reset_index(drop=True)
            elif group == 'Weekly':
                ds_h = cls._load_data(group='Hourly', min_n_instances=min_n_instances)
                ds_d = cls._load_data(group='Daily', min_n_instances=min_n_instances)
    ...
    @classmethod
    def _load_data(cls, group, min_n_instances=None):
